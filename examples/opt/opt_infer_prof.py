...
def main(tokenizer, engine, args):
    try:
        ...
        _warm_up(input_text, inputs, engine, args, 1)
        prof_collect.set_path(args.cpu_log_path)
        gen_res = prof_collect(input_text, inputs, engine, args, 0)
        return gen_res
    finally:
        engine.shutdown()
...
def prof_collect(input_text, inputs, engine, args, try_time):
    try_time += 1
    _gen_fn(input_text, inputs, engine, args, try_time)
    p_start()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cpu_log_path', type=str, default="tmp_cpu_log.txt")
